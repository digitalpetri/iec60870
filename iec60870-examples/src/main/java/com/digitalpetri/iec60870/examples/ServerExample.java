...
public final class ServerExample {
   ...
   * server and must {@link Iec60870Server#close() close} it.
   *
   * @param port the TCP port to bind on the loopback interface.
   * @return the started server.
   */
  public static Iec60870Server start(int port) {
    Station station =
    ...
    ScheduledExecutorService publisher =
        Executors.newSingleThreadScheduledExecutor(
            runnable -> {
              Thread thread = new Thread(runnable, "iec60870-example-publisher");
   ...
   */
  private static void publishEveryType(Iec60870Server server, int tick) {
  ...
  public static void main(String[] args) {
    Iec60870Server server = start(2404);
    Runtime.getRuntime()
        .addShutdownHook(
            new Thread(
                () -> {
