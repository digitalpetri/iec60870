 ...
 * ApciSession}.
 *
 * <p>A {@code BalancedEngine} owns the stop-and-wait flow control for one point-to-point serial
 ...
 * supplied {@link Ft12LinkLayer.Output}, inbound frames are fed in via {@link #onFrame(Ft12Frame)},
 * and delivered application ASDUs plus lifecycle events are reported through the supplied {@link
 * Session.Events}. The link layer never touches a serial port or a {@code ByteBuf} directly.
 *
 ...
 * receives, tracking the peer's FCB) at the same time, regardless of {@link Ft12LinkLayer.Role}.
 * The role selects only the small asymmetries: the DIR bit on every outgoing frame, which station
 * initiates the link-reset bring-up, and which station may drive {@link
 * #startDataTransfer()}/{@link #stopDataTransfer()}.
 *
 ...
 * send application data with {@link #sendAsdu(Asdu)}. A {@link Ft12LinkLayer.Role#CLIENT} drives
 * the link-reset bring-up with {@link #startDataTransfer()}, which completes when the link becomes
 * available; a {@link Ft12LinkLayer.Role#SERVER} reaches the available state by receiving the
 * peer's reset-of-remote-link frame. Call {@link #close()} when the transport goes away; it is
 * idempotent.
 *
 ...
 * the link layer and should not block.
 */
final class BalancedEngine implements Ft12Engine {

  private static final Logger LOGGER = LoggerFactory.getLogger(BalancedEngine.class);
  ...
  private final Ft12LinkLayer.Role role;
  ...
  BalancedEngine(
    ...
    if (settings.mode() != LinkMode.BALANCED) {
      throw new IllegalArgumentException(
          "BalancedEngine supports BALANCED mode only, got: " + settings.mode());
    }
   ...
   * @throws IllegalStateException if called on a {@link Ft12LinkLayer.Role#SERVER} station.
   */
  @Override
  public CompletionStage<Void> startDataTransfer() {
    lock.lock();
    try {
      if (role != Ft12LinkLayer.Role.CLIENT) {
