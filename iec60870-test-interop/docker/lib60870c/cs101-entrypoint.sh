...
#
#   listen mode:  socat PTY,link=/dev/ttyCS101  <->  TCP-LISTEN:2404
#   connect mode: socat PTY,link=/dev/ttyCS101  <->  TCP:<host>:<port>
#
# It creates a pseudo-terminal, symlinks its slave side to /dev/ttyCS101 (the
# device the C peer opens). In listen mode it exposes the other end as
# TCP-LISTEN:2404 (the container port Testcontainers maps to the host). On the
# host side the serial tests run their own
# `socat PTY,link=<host pty> TCP:localhost:<mappedPort>`, so the full octet path
# is:
#
#   Java <-> host PTY <-> host socat <-> TCP <-> container socat <-> /dev/ttyCS101 <-> C peer
#
# In connect mode the container bridges directly to a Java TcpIec101Server
# published through Testcontainers.exposeHostPorts:
#
#   C peer <-> /dev/ttyCS101 <-> container socat <-> TCP host.testcontainers.internal:<port> <-> Java
#
...
#   INTEROP_CS101_BRIDGE   listen (default) | connect
#   INTEROP_CS101_TCP_HOST TCP host for connect mode (default host.testcontainers.internal)
#   INTEROP_CS101_TCP_PORT TCP port to listen on or connect to (default 2404)
...
BRIDGE="${INTEROP_CS101_BRIDGE:-listen}"
TCP_HOST="${INTEROP_CS101_TCP_HOST:-host.testcontainers.internal}"
TCP_PORT="${INTEROP_CS101_TCP_PORT:-2404}"

case "${BRIDGE}" in
  listen)
    TARGET="TCP-LISTEN:${TCP_PORT},reuseaddr,nodelay"
    echo "INTEROP-CS101-BRIDGE starting socat PTY ${DEVICE} <-> ${TARGET} (b${BAUD})"
    ;;
  connect)
    TARGET="TCP:${TCP_HOST}:${TCP_PORT},nodelay"
    echo "INTEROP-CS101-BRIDGE starting socat PTY ${DEVICE} <-> ${TARGET} (b${BAUD})"
    ;;
  *)
    echo "INTEROP-CS101-BRIDGE FAILED: unknown INTEROP_CS101_BRIDGE=${BRIDGE}"
    exit 1
    ;;
esac

# Bridge the PTY (whose slave side is symlinked to ${DEVICE}) to the selected TCP endpoint.
...
echo "INTEROP-CS101-BRIDGE ${DEVICE} ready; launching peer (mode=${INTEROP_CS101_MODE:-balanced} role=${INTEROP_CS101_ROLE:-slave} bridge=${BRIDGE})"
