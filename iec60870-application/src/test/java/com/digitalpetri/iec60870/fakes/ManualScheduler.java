...
/**
 * A deterministic, single-threaded {@link ScheduledExecutorService} backed by a virtual clock, for
 * driving facade request/command timeouts without wall-clock sleeps.
 *
 ...
 * synchronously, in deadline order, on the advancing thread. Combined with a direct (same-thread)
 * callback executor, this collapses the scheduler-thread/callback-thread hop to a single
 * deterministic thread so a timed-out request is failed and cleaned up before {@code advance}
 * returns.
 *
 * <p>{@link #execute(Runnable)} runs inline on the caller thread. Only the scheduling members the
 * facade uses are implemented; the remaining {@link ScheduledExecutorService} members throw {@link
