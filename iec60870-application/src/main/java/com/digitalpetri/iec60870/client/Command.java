 ...
 *
 * <p>A {@code Command} is distinct from the wire-level {@code
 * com.digitalpetri.iec60870.asdu.object} command records: it carries the target {@link
 * PointAddress}, the command value, and the command options (the qualifier and an optional
 * CP56Time2a time tag), but it does not fix the select/execute flag. The {@link CommandService}
 * chooses select or execute from the {@link CommandMode} when it builds the ASDU. When a command
 * carries a {@link #time()}, the time-tagged ASDU type (for example C_SC_TA_1) is used; otherwise
 * the untimed type (for example C_SC_NA_1) is used.
 *
