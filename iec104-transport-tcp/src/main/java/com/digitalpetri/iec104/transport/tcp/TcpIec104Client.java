...
import com.digitalpetri.iec104.client.ClientConfig;
import com.digitalpetri.iec104.client.DefaultIec104Client;
...
/**
 * User-facing entry point that builds a TCP/TLS {@link Iec104Client}.
 *
 * <p>This is the transport-module factory the proposed API sketches as {@code
 * Iec104Client.builder().remote(host, port).tls(..)}. Because transport wiring (host, port, TLS,
 * event loops) must not appear in core, those knobs live here; {@link Builder#build()} constructs a
 * {@link NettyClientTransport} plus a {@link DefaultIec104Client} and returns the core {@link
 * Iec104Client} interface, whose every protocol method matches the proposed API exactly.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 ...
 * javax.net.ssl.SSLContext}; {@link Iec104Client#connect()} then completes only after the TLS
 * handshake succeeds.
 */
public final class TcpIec104Client {
  ...
  /**
   * A builder for a TCP/TLS-backed {@link Iec104Client}.
   *
  ...
  public static final class Builder {
     ...
     *
     * <p>When set, {@link Iec104Client#connect()} completes only after the TLS handshake succeeds.
     *
    ...
    /**
     * Builds the Netty transport and the core client and returns the {@link Iec104Client}.
     *
     * @return the configured client.
     */
    public Iec104Client build() {
      ...
      return new DefaultIec104Client(transport, clientConfigBuilder.build());
    }
  }
}
