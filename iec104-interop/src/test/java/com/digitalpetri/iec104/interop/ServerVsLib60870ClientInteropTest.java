 ...
 * Java {@link Iec104Server} (controlled station) over plaintext IEC 60870-5-104.
 *
 ...
 * peer runs the scripted sequence documented in {@code iec104-interop/docker/INTEROP-CONTRACT.md}
 * section 11 and prints {@code PASS:}/{@code FAIL:} markers plus a final {@code INTEROP-CLIENT
 * RESULT pass=<n> fail=<n>} line; this test asserts on those markers.
 *
 * <h2>Wiring</h2>
 *
 * <p>The Java server binds {@code 127.0.0.1} on an ephemeral port (chosen with a throw-away {@link
 * ServerSocket} so the concrete number is known up front, since {@link Iec104Server} exposes no
 ...
 * iec104-interop/docker/lib60870c} via {@link ImageFromDockerfile} so the test is self-contained
 * (the Docker layer cache makes reruns fast; the first run shallow-clones and builds lib60870-C,
 * which can take minutes). The client container is one-shot: it runs the script and exits, so its
 * startup uses {@link OneShotStartupCheckStrategy} (which blocks {@code start()} until the process
 * exits, bounded by the startup timeout) and its full stdout is captured with a {@link
 * ToStringConsumer}. A non-zero client exit (returned when {@code fail != 0}) surfaces as a {@link
 * ContainerLaunchException}, which is caught so the captured log drives a readable assertion.
 *
